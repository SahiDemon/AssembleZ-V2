...
import ProductCard from "@/components/assemblez/product-card"; // Import your specific card here
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox"; // Assuming you have shadcn checkbox or similar
import { 
  Cpu, 
  SlidersHorizontal, 
  ChevronDown, 
...
interface CpuProduct {
  id: string;
  name: string;
  specs: string; // Pre-formatted for the card (e.g., "8 Cores, 4.2GHz")
  price: string;
  retailer: string;
  image: string;
  trend?: { direction: "up" | "down"; amount: string };
  brand: "Intel" | "AMD"; // For filtering logic
}
...
export default function CpuPage() {
  ...
  return (
    <main ref={containerRef} className="relative min-h-screen w-full text-white selection:bg-primary/30 font-sans">
      <div className="relative">
            ...
            <aside className={`filter-sidebar w-full lg:w-72 shrink-0 space-y-6 ${isFilterOpen ? 'block' : 'hidden lg:block'}`}>
              <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-6 sticky top-24">
                  ...
                  <div className="space-y-3">
                    {["Intel", "AMD"].map((brand) => (
                      <div key={brand} className="flex items-center space-x-3">
                        {/* Use native checkbox if shadcn not available, styling roughly below */}
                ...
                <div className="space-y-4 border-t border-white/10 pt-6 mt-6">
                 ...
                 {/* Price Range Filter (Visual Only) */}
                 <div className="space-y-4 border-t border-white/10 pt-6 mt-6">
        ...
        {/* Footer Area - Reusing the footer style from main page */}
